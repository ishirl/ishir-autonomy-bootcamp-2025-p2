...
def main() -> int:
    ...
    status_queue = queue_proxy_wrapper.QueueProxyWrapper(mp_manager, STATUS_QUEUE_MAX_SIZE)

    # Just set a timer to stop the worker after a while, since the worker infinite loops
    threading.Timer(
        HEARTBEAT_PERIOD * (NUM_TRIALS * 2 + DISCONNECT_THRESHOLD + NUM_DISCONNECTS + 2),
        stop,
        (controller, status_queue),
    ).start()

    # Read the main queue (worker outputs)
    threading.Thread(target=read_queue, args=(status_queue, main_logger), daemon=True).start()
